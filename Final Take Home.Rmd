...
find_error<- function(){
  ...
  mean_resample<-mean(resample)
  sd_resample<- sd(resample)
  Serror<- sd_resample/sqrt(sample_size)
  error
}


left<-replicate(num_samples, find_error())
right<-replicate(num_samples, find_error())
